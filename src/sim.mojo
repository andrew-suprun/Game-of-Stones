...
fn sim_opening[T1: TTree, T2: TTree](name1: String, time1: UInt, name2: String, time2: UInt, opening: List[String]) raises -> String:
    ...
    while True:
        ...
        if turn == first:
            ...
            score = result.score
            print("  ", name1.ascii_ljust(name_size), String(result.move).ascii_ljust(8), String(result.score).ascii_rjust(7), "  ", (perf_counter_ns() - start) / 1_000_000_000, "s", sep="")
        else:
            var result = t2.search(g2, time2)
            move = String(result.move)
            score = -result.score
            print("  ", name2.ascii_ljust(name_size), String(result.move).ascii_ljust(8), String(result.score).ascii_rjust(7), "  ", (perf_counter_ns() - start) / 1_000_000_000, "s", sep="")
