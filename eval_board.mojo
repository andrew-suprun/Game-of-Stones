...
def main() raises:
    ...
    for move in open_moves:
        ...
        for p in move_places:
            ...
            var min_offence = max(value_offsence / 4, 30)
            var min_defence = max(value_defence / 4, 30)
            print(t"  place_offsence={place_offsence} value_offence={value_offsence} min_offence={min_offence}")
            print(t"  place_defence={place_defence} max_value={value_defence} min_defence={min_defence}")
            for y in range(board_size):
                for x in range(board_size):
                    value_offsence = 0
                    value_defence = 0
                    ...
                    for dir in range(4):
                        value_offsence = max(value_offsence, values[dir][turn])
                        value_defence = max(value_defence, values[dir][1 - turn])
                    if (value_offsence >= min_offence or value_defence >= min_defence) and board._places[y * board_size + x] == Board.empty:
