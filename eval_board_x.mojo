...
from engine import BoardX, PlaceX
...
#     " q12-p7 n16-o15 s:452.0 l18-r12 m15-n15 s:1672.0 k15-p15 n18-o17 s:277.0 p18-n17 o14-r14 s:455.0 q14-o16 p16-m19"
# )

comptime script = "b1-d1-f1-h1 j1 e1"
...
def main() raises:
    var board = BoardX[size, win_stones]()
    ...
    for move in open_moves:
        ...
        for place in move_places:
            print(t"place: {place} {"X" if turn == 0 else "O"}")
            var score = BoardX.PlaceScores(fill=BoardX.PlaceStoneCounts(fill={0, 0}))
            board.score_place(PlaceX(String(place)), score)
            print(t"black: {score[0]} | white: {score[1]}")
            board.place_stone(PlaceX(String(place)), turn)
            print(board)
