import Foundation
 ...
 @inlinable 
 public func heapAdd<T>(_ item: T, to items: inout [T], maxItems: Int, less: (T, T) -> Bool) {
...
@inlinable 
func siftUp<T>(_ items: inout [T], _ less: (T, T) -> Bool) {
...
@inlinable 
func siftDown<T>(_ items: inout [T], _ less: (T, T) -> Bool) {
    var idx = 0
    let elem = items[idx]
    while true {
        var first = idx
