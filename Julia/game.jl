...
function top_moves(game, ::Name{:Gomoku}, moves)
    ...
    for place in game.places
        ...
        if value >= win_value
            empty!(moves)
            push!(moves, MoveValue(Move(place, place), win_value, first_win))
            return
        elseif value <= -win_value
            empty!(moves)
            push!(moves, MoveValue(Move(place, place), -win_value, second_win))
            return
        end
...
function top_moves(game, ::Name{:Connect6}, moves)
    ...
    for (i, place1) in enumerate(game.places)
        ...
        if value1 <= -win_value
            empty!(moves)
            push!(moves, MoveValue(Move(place1, place1), game.value + value1, second_win))
            return
        elseif value1 >= win_value
            empty!(moves)
            push!(moves, MoveValue(Move(place1, place1), game.value + value1, first_win))
            return
        end
        ...
        for place2 in game.places[i+1:end]
            ...
            if value2 <= -win_value
                push!(moves, MoveValue(Move(place1, place2), game_value + (value1 + value2) ÷ Int16(2), second_win))
                place_stone!(game, place1, -1)
                return
            elseif value2 >= win_value
                push!(moves, MoveValue(Move(place1, place2), game_value + (value1 + value2) ÷ Int16(2), first_win))
                place_stone!(game, place1, -1)
                return
            end
